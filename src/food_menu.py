...
class FoodMenu:
    ...
    def increase_item_count(self, user_name, item_name):
        update_query = f"update {self.cart_table} set item_count = item_count+1 where user_name='{user_name}' and item_name = '{item_name}'"
        self.execute_query(update_query)
    
    def decrease_item_count(self, user_name, item_name):
        get_query = f"select item_count from {self.cart_table} where user_name = '{user_name}' and item_name = '{item_name}'"
        item_count = self.execute_get_query(get_query)
        if item_count>2:
            update_query = f"update {self.cart_table} set item_count = item_count-1 where user_name='{user_name}' and item_name = '{item_name}'"
            self.execute_query(update_query)
